...
// N = SampleRate / (приблизна роздільна здатність). 24000 / 93.75 = 256.
...
private:
    ...
    // Приклад розрахунку для Fs=24000, N=256, f_space=1275, f_mark=1445:
    // k_space = 1275 * 256 / 24000 = 13.6
    // k_mark = 1445 * 256 / 24000 = 15.4133
    // coeff = 2 * cos(2 * PI * k / N) * 2^15
    // coeff_space_Q15 = round(2 * cos(2 * PI * 13.6 / 256) * 32768) = 61967
    // coeff_mark_Q15 = round(2 * cos(2 * PI * 15.4133 / 256) * 32768) = 60964
    static const int32_t coeff_space_Q15 = 61967; // ПРИКЛАД КОНСТАНТИ
    static const int32_t coeff_mark_Q15 = 60964;  // ПРИКЛАД КОНСТАНТИ
